...
|        内置函数、视图、触发器等 |   │
+-------------------------------+   │
|        存储引擎抽象层 (Handler API)│ ←─ Server 层（通用逻辑）
+===============+===============+   │
                |                   │
    +-----------v-----------+       │
    |      InnoDB 引擎       |       │
...
MySQL 的通用服务层，与存储引擎无关，所有引擎共用，主要有以下职责：

**连接管理：**
- 处理客户端连接（TCP/IP、Unix Socket、Named Pipe）；
- 验证用户名/密码、权限；
- 为每个连接分配一个独立线程（或使用线程池）；
- 管理连接状态、会话变量（如 autocommit、time_zone）。

注意：MySQL 默认为每个连接创建一个线程，高并发时需配置 thread_pool 避免 OOM。

**SQL 接口：**
- 接收 SQL 语句（如 SELECT, INSERT, CREATE）；
- 判断语句类型（DML/DDL/DCL）；
- 调用后续解析器。

**查询解析器：**
- 词法分析：将 SQL 字符串拆分为 token（如 SELECT, id, FROM, users）；
- 语法分析：构建解析树（Parse Tree），验证 SQL 是否符合语法规则，若语法错误，直接返回 ERROR 1064。

**查询优化器：**
这是 Server 层最复杂的模块，目标是找到成本最低的执行计划。

主要优化策略：
- 索引选择：哪个索引更快？是否走全表扫描？
- JOIN 顺序重排：A JOIN B JOIN C → 哪种顺序 I/O 最少？
- 子查询优化：转为 JOIN 或物化；
- 谓词下推：把 WHERE 条件尽可能下推到存储引擎；
- 常量折叠：WHERE id = 1 + 2 → WHERE id = 3。

输出：执行计划（Execution Plan），可通过 EXPLAIN 查看。

**执行器（Executor）：**
- 根据优化器生成的执行计划，调用存储引擎 API 逐行获取数据；
- 处理事务、权限检查（如 SELECT 是否有列权限）；
- 执行内置函数（如 NOW(), CONCAT()）、触发器、视图逻辑；
- 将最终结果集返回给客户端。

**其他 Server 层功能：**
- 权限系统：基于 mysql.user 表的访问控制；
- 缓存（已废弃）：MySQL 8.0 移除了 Query Cache；
- Binlog：记录逻辑日志（用于复制、恢复），属于 Server 层；
- 分区表逻辑：分区裁剪、路由由 Server 层处理。

#### （2）存储引擎层

这是 MySQL 的可插拔底层，负责数据的实际存储、检索、事务、锁等。Server 层通过统一的 **Handler 抽象类** 与存储引擎交互，Server 层不关心数据如何存储，只调用这些接口；引擎决定是用 B+Tree、Hash、LSM-Tree 还是内存结构。

![alt text](/复习资料/数据库/img/image-5.png)

### （2）存储引擎

MySQL 常见的存储引擎：

![alt text](/复习资料/数据库/img/image-6.png)

#### （1）InnoDB 存储引擎详解

**核心原理：**
- 数据组织：采用 聚集索引（Clustered Index） 结构；
  - 主键索引的叶子节点直接存储整行数据（聚簇索引）；
  - 非主键索引（二级索引）的叶子节点存储主键值，需回表查询。

- 事务支持：基于 MVCC（多版本并发控制） + Undo Log + Redo Log 实现 ACID；

- 锁机制：支持 行级锁（Record Lock）、间隙锁（Gap Lock）、临键锁（Next-Key Lock）；

- 崩溃恢复：通过 Redo Log（重做日志） 保证持久性，Undo Log（回滚日志） 保证原子性和 MVCC。

**典型使用场景：**
- 电商系统（订单、库存、支付）；
- 银行交易系统；
- 任何需要高并发写入 + 数据强一致的 OLTP 场景。
...
- 无事务：所有操作立即写入磁盘（或 OS 缓存），无日志保障，SELECT 极快（尤其全表扫描），高速读取。且由于无缓冲池、无事务日志，低内存占用

- 锁机制：仅支持 表级锁（Table-level Locking）；

- 崩溃恢复：不支持！表损坏后需手动 REPAIR TABLE。

严重缺陷：
- **写操作阻塞全表**：一个 UPDATE 会锁住整个表，其他读写全部等待；
- **崩溃即丢数据**：断电后 .MYD/.MYI 可能损坏，且无法自动恢复；
- **不支持事务**：无法回滚，无法保证数据一致性。

**典型使用场景：**

- 只读或极少更新的报表表（如日志归档、统计数据）；
- 全文搜索需求且无法升级到 MySQL 5.6+（现代应用应改用 Elasticsearch）；
- 内存敏感、追求极致读性能的边缘场景。
