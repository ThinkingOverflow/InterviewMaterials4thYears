...
参考文章：https://juejin.cn/post/7361687968518668322#heading-23

## 4、NATS

定位：云原生微服务通信基础设施，强调简单、快速、可扩展。

![alt text](/复习资料/消息队列/img/image-9.png)

### （1）核心特性

NATS Core（无状态消息总线），纯内存 Pub/Sub + Request/Reply，其特点是：
- 无持久化：消息只在内存中传递，Consumer 不在线则丢失；
- 超低延迟：端到端 < 1ms（Go 协程 + 零拷贝）；
- 自动发现：支持 cluster 模式，节点自动组网；
- 协议极简：文本协议（类似 HTTP），易调试；
- 多语言 SDK：官方支持 40+ 语言。

适用场景：服务间实时通知、RPC 调用、IoT 设备指令下发（不要求可靠投递）。

NATS JetStream（持久化消息系统）：JetStream 是 NATS 的“消息队列”能力载体，对标 Kafka/RocketMQ。其核心能力：

- 持久化存储：消息写入磁盘（支持文件/WAL），可配置保留策略（时间/大小/数量）
- 流（Stream）模型：类似 Kafka Topic，但更灵活（支持按 subject 自动分片）
- 消费者（Consumer）：支持 Push/Pull 模式，可指定起始位置（时间/序列号）
- Exactly-Once：通过 msg ID 去重实现（需 Producer 显式设置）
- 消息回溯：按时间或序列号重放历史消息
- 高可用：基于 RAFT 共识（内置，无需外部依赖）
- 水平扩展：Stream 可跨节点分片（Super Cluster）

### （2）NATS vs Kafka vs RocketMQ 深度对比

#### （1）3种 MQ 的差异如下：

| 维度 | NATS (JetStream) | Kafka | RocketMQ |
|------|------------------|-------|----------|
| 设计哲学 | 云原生、极简、低开销 | 高吞吐日志管道 | 业务消息引擎 |
| 部署复杂度 | ⭐ 极低（单二进制，无外部依赖） | ⭐⭐⭐ 高（需 JVM + KRaft/ZK） | ⭐⭐ 中（需 NameServer + Broker） |
| 语言栈 | Go（轻量） | JVM（资源消耗大） | JVM |
| 持久化模型 | 流（Stream） + 消费者组 | 分区日志（Partition Log） | Topic/Queue + CommitLog |
| 消息可靠性 | RAFT 多副本 + ACK | ISR + acks=all | 同步双写 / DLedger |
| 吞吐量 | 万～十万级 TPS | 百万级 TPS | 十万级 TPS |
| 延迟 | 微秒级（Core）<br>毫秒级（JetStream） | 毫秒级 | 毫秒级 |
| 顺序消息 | Stream 内有序 | Partition 内有序 | Queue 内有序 |
| 延迟消息 | ❌ 不支持（需外部调度） | ❌ 不支持 | ✅ 内置 18 级延迟 |
| 事务消息 | ❌ 不支持 | ⚠️ 仅 Producer 跨分区原子写 | ✅ 分布式事务消息 |
| 消息过滤 | ✅ Subject 层级天然过滤<br>（如 `ORDERS.*`） | ❌ 仅客户端过滤 | ✅ Tag / SQL 服务端过滤 |
| 重试机制 | ❌ 无内置（需应用层实现） | ❌ 无内置 | ✅ 自动重试 + 死信队列 |
| 协议 | 自研文本协议（简单） | 自研二进制协议 | 自研二进制协议 |
| 云原生友好度 | ⭐⭐⭐⭐⭐（Sidecar 模式、Operator） | ⭐⭐（资源重） | ⭐⭐⭐（有 Operator） |
| 典型场景 | 微服务通信、IoT、边缘计算 | 日志、流处理、大数据 | 金融交易、电商订单 |

#### （2）关键差异解析

**架构复杂度**
- NATS：单进程搞定一切（JetStream 内置 RAFT），适合 Kubernetes Sidecar 模式；
- Kafka/RocketMQ：多组件（Controller/Broker/NameServer），运维成本高

**消息模型**
- NATS：基于 Subject（主题层级），天然支持通配符（foo.*.bar）；
- Kafka/RocketMQ：扁平 Topic，过滤靠 Tag/Header。

**持久化 vs 非持久化**
- NATS Core 不持久化 → 适合 fire-and-forget 场景；
- JetStream 提供持久化，但功能不如 Kafka/RocketMQ 完善（如无延迟消息）。

**性能 vs 功能**
- NATS 在延迟和资源占用上碾压 JVM 系消息队列；
- 但在高级业务特性（事务、延迟、重试）上远弱于 RocketMQ。

![alt text](/复习资料/消息队列/img/image-10.png)