...
- 再交给 LoadBalancer 选一条资源，你当前 SIT 配置是 chat.loadBalancer=quotaBased，即配额优先策略
  - quotaBased 细节：先看 Redis 中该模型哪些 key 还有可用配额；若都超限则降级到候选集内“加权+粘性/随机”；粘性是 userId#modelKey 哈希，权重来自 weight（默认 1）
    - 这里选还有s可用配额的模型，选取 score >= maxTokens（maxTokens 不是固定值，取决于每次请求：常见是“**预估输入 token + 请求的输出上限**“）
...
下游返回 429 且错误码是账号级限流时，先把该资源打入冷却，再抛 OpenAiPoolRetryableException。入口层捕获后只重试一次，并在响应尚未提交时 reset() 后重调，这次会重新调度到其他资源。冷却实现是把资源从容量候选集中暂时移除。
...
#### （1）对接厂商时候需要关注的点
对接不同厂商时，不是简单改 URL，而是要适配：

- 认证方式（apikey 或者是 客户端认证）
- 网络环境（是否需要海外网络）
- 协议路径（流式、同步、异步）
- 重试和容错策略（不同厂商的重试和容错策略不同）

#### （2）标准 OpenAI 协议兼容：`BaseOpenAiImpl` 的作用
...
**平台要让业务方用“标准 OpenAI 风格接口”，但后台真实厂商可能并不真的是 OpenAI，这就是这层存在的意义。**
...
这一步决定它落到 `mip-chat-app` 还是 `aigc-multimedia`。
...
如果是 OpenAI 兼容协议，可优先复用 `BaseOpenAiImpl`。
...
如果模型能力和标准协议差异大，一般要保留专属入口。

#### 第 6 步：根据模型的返回对请求明细信息进行特殊处理

要确认：

- usage 是否有返回
- 没返回时是否支持离线估算
- 计费口径是 token、时长、次数、图片张数还是字符数
...
> 最近接入 OpenAI 兼容模型时，我们不是简单新增了一个 Controller，而是把这条调用链纳入了平台统一标准：前面是鉴权和权限校验，中间是 OpenAI 兼容协议适配，后面是账号池/席位模式调度、429 冷却重试和调用数据落库。这条链路很能体现平台化思路。

### Q19：grafana 数据面板相关原理

#### （1）Grafana + Prometheus + chat-app metrics 暴露的工作原理

流程：
- 应用代码里埋点（Counter/Histogram）。
- 应用暴露 HTTP metrics 端点（如 /metrics）。
- Prometheus 时序数据库定时拉取（scrape）这个端点，存成时序数据。即其作用是：**收集 + 存储大模型调用各项监控数值**。
- 数据源选择 Prometheus，用 PromQL 写查询语句。即其作用是：**把这些数值做成实时可视化监控大屏**。

组件职责：
- 应用（mip-chat-app）：生产指标、暴露指标。
- Prometheus：采集+存储+查询时序指标。
- Grafana：图表展示、仪表盘、告警面板。

#### （2）Prometheus 时序数据 vs 常见报表数据

- 时序数据：指标名 + labels + 时间戳 + 值，高频采样，偏运维观测（QPS、延迟、错误率）。
- 报表数据：按日/月聚合后的业务统计，偏经营分析（费用账单、部门归因）。

Prometheus 看“系统此刻健康和趋势”，报表库看“业务结算与管理口径”。

#### （3）这个项目里如何暴露 metrics，为什么这么做

- Counter：只增不减的累计计数器。适合统计总请求数、成功数、失败数、token累计量。
- Histogram：把观测值按桶分布统计。适合统计延迟和每次请求 token 分布（可算 P95/P99、分位趋势）。

这里涉及的 counter 和 histogram 包含：
- RPM（每分钟请求数）
- TPM（每分钟 token 数）
- 请求调用耗时
- 缓存命中率
...
**总结**：
项目里通过 LlmMetricsFacade 在请求链路关键节点做实时埋点：请求进入时挂载上下文（app_id、model、是否流式），请求结束后根据成功/失败分别上报 Counter（请求数、token 总量累计），并对缓存命中率、RPM、TPM 等指标用 Histogram 观测。

应用再通过 /metrics（GET ${prometheus.metrics.path:/metrics}）以 Prometheus 文本格式暴露当前指标快照。Prometheus 按配置周期主动 pull 该端点，落成带时间戳的时序数据；

Grafana 以 Prometheus 为数据源，通过 PromQL 做聚合、分组和分位计算，展示模型维度的 QPS、成功率、错误率、P95 延迟、token 消耗、缓存命中等看板。

### Q20：旧架构问题

#### （1）问题 1：控制面和数据面的抽象不够彻底

老架构虽然分了 `admin/app/multimedia`，但：

- 模型适配逻辑还是比较 case-by-case，新能力容易继续堆在 `mip-chat-app`网关
- **资源治理、路由策略**与代码耦合较深，没有实现控制面可统一配置；插件能力没有彻底平台化，**缓存、认证、日志、metrics、智数上报**这类横切能力还在应用服务里做，没有统一下沉到网关插件层，所以复用、灰度、多版本管理都不够好。各种逻辑耦合在同一个服务里，扩展成本越来越高

#### （2）问题 2：网关层偏流量转发，不够智能
...
并没有形成独立的可插拔网关能力体系，而是大量写在应用代码或旁路服务里。

#### （3）### 问题 3：未来要支持更多新能力

平台未来不只是“模型代理”，而是要继续承接：

- MCP 服务
- A2A 服务
- Skills 服务
- AI Route / Provider 控制面
- 插件体系
- 更完整的可观测能力
- token 运营与预算管理
...
老架构虽然已经拆分为 admin / app / multimedia 三个服务，但整体上仍然是“能跑起来”的工程化整合，还没有真正完成**控制面、数据面和网关横切能力的彻底平台化**：
- 一方面，模型适配、资源治理、路由策略等能力仍然和 mip-chat-app 代码强耦合，新能力容易继续堆在在线网关里；
- 另一方面，缓存、认证、日志、metrics、智数上报等 AI 横切能力也没有统一下沉到可插拔的网关插件层，导致复用、灰度和多版本管理能力不足。
- 与此同时，平台未来还要继续承接 MCP、A2A、Skills、AI Route / Provider 控制面、插件体系、完整可观测和 token 运营预算等更复杂能力，如果继续沿用老架构叠加实现，系统复杂度和扩展成本都会持续上升。

### Q21：为什么旧架构设计为这个样子

#### （1）原因 1：时间紧、任务重，先求能接通

当时平台处于需求快速爆发期，最现实的目标是：
- 尽快对接多个厂商
- 尽快让业务方统一调用
- 尽快接上基础计费和治理

#### （2）原因 2：外部模型生态变化快，标准不成熟
...
都不一致，这导致平台早期很难一次性设计出完美标准，只能先以 **统一入口 + 多适配器** 的方式快速落地。

#### （3）原因 3：当时外部条件还不成熟

当时之所以没有一开始就做成现在这种“AI 网关 + 插件层 + 独立调度中台”的架构，一个很现实的原因是，当时还没有足够成熟、能直接拿来承接这类场景的开源 AI 网关体系，插件能力也不成熟，很多能力没有现成基础设施可以依赖。

所以平台只能先采用更务实的做法：把统一接入、模型适配、治理控制、部分路由和统计能力先集中放在业务服务里实现，优先解决“先接通、先可用、先可治理”的问题。

等后面 Higress、WASM 插件、独立调度数据面这些基础条件逐渐成熟之后，才有条件把原来耦合在应用里的能力一步步拆出来，演进成现在的新架构。

#### （4）总结

旧架构之所以设计成现在这个样子，本质上是一个典型的阶段性产物：
- 当时平台正处于需求快速爆发期，首要目标是尽快对接多个厂商、尽快让业务方统一接入、尽快把基础计费和治理能力补上，所以架构上优先选择了能够快速交付的集中式实现；
  
- 同时，外部大模型生态变化很快，不同厂商在鉴权方式、参数协议、流式返回、视频任务模型、usage 字段等方面差异很大，平台很难一开始就抽象出一套完美标准，只能先用“统一入口 + 多适配器”的方式落地；
  
- 再加上当时还没有足够成熟的开源 AI 网关、插件体系和独立调度底座可直接复用，因此很多统一接入、模型适配、治理控制、部分路由和统计能力只能先收敛在业务服务里实现，优先保证先接通、先可用、先可治理，等后续 Higress、WASM 插件、独立调度数据面这些基础设施逐步成熟后，再把原来耦合在应用中的能力拆出来，演进成现在的新架构。
...
chatgpt_charging_main
清洗后的计费明细表，还是明细粒度，但补齐了组织/计费口径

chatgpt_charging_day
按 用户 + 模型 + 系统 + 日期 聚合的日汇总表，偏分析看板

chatgpt_daily_cost_aggregation
按 系统 + 模型 + 日期 + 模型类型 聚合的成本账单表，偏限额/对账/总费用治理
