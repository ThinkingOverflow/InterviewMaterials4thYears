...
系统数目：500，当前 GNC 真实的调度任务数目在 100000 左右，真实的调度日志数目（100 个表）在 2 个亿左右。
...
这里，热点池按业务方需求进行宽松手动维护分配；普通池按简单哈希 + 当前负载校验（ 单标记录数/慢 SQL 情况/已绑定系统数）选表。命中阈值后该表停止新分配，切到下一个候选表。
...
##### （1）为什么 Paas dispatch-api 只能固定在 4 台宿主机上？如果 Pod 数量超过 4 个，可不可以？

Paas 早期没有 Hub，dispatch-api 需要直连多个网络区域的 worker，所以在 4 台机器上沉淀了大量防火墙策略。后续由于网络团队变更，部分策略不可追溯，新增宿主机成本和风险很高，因此只能在这 4 台机器内扩容。
K8s 层面 Pod 超过 4 个“可以”，但这套 Akka 调度架构更适合一机一实例；多 Pod 同机会带来端口/故障域/分片复杂度问题，生产通常仍保持 4 Pod 对应 4 宿主机。

##### （2）dispatch-api 是按 PodIP 还是 NodeIP 做实例识别和任务分片？心跳和分片流程是什么？

逻辑上是按 Akka 成员地址（host:port）识别实例并分片；在你们部署形态下，host 基本就是宿主机 NodeIP，所以 cluster:nodes 看起来是 NodeIP 列表。
每个 API 实例都会周期上报 TrigInst 心跳到 djs_job_registry（不是只有抢到锁的实例才上报）。当 MemberUp/MemberDown 事件发生时，多个节点会竞争分布式锁，抢到锁的节点执行本轮重分片，更新 Redis 的 cluster:nodes 和 job:active:{node}，各实例只调度自己的分片任务。

##### （3）在 Paas 的 Akka 集群里，什么是种子节点？它和普通节点有什么区别？种子节点是怎么确定的，为什么 4 个调度节点通常只配 3 个种子节点而不是 4 个都配？

种子节点是 Akka 集群的“入群引导地址”，新节点启动时先连 seed 列表中的任一节点来完成集群发现。它的职责是引导入群，不是固定 master，也不等于 leader。leader 是运行时动态选举出来的。
种子节点一般在部署阶段通过 akka-*.conf 静态配置，属于运维/架构层面的预设，不是运行时自动决定。
4 节点通常配 3 个 seed 是工程上更稳妥的折中：选 3 台最稳定节点做入口，保证可用性，同时降低配置维护复杂度和误配风险。第 4 台作为普通工作节点即可，仍然能正常参与调度与分片。

##### （4）akka 集群脑裂是什么意思？

Akka 集群“脑裂”就是：**原本一个集群因为网络分区，被切成两个（或多个）彼此都认为自己是正常集群的子集群。**

举个你们场景的例子：
- 原来 4 个节点：A、B、C、D。
- 网络抖动后，A 看不到 B/C/D，B/C/D 也看不到 A。
- 两边都继续运行并各自选出 leader。
- 结果变成两个“脑”：
  - 子集群1：A（认为自己是完整集群）
  - 子集群2：B/C/D（也认为自己是完整集群）

在调度系统里会带来：

- 分片状态不一致（cluster:nodes/job:active:* 可能被不同分区各自改写）
- 新增/启停任务只在某一边生效
- 任务倾斜、漏调度或重复调度风险上升

一句话：**脑裂不是单点挂掉，而是“多边都活着但互相看不见”，最容易把分布式状态搞乱。**

#### （2）故障过程 + 出现的问题 + 解决方案

**故障过程：**
- Paas dispatch-api 4 节点中，172 因网络问题与其余节点隔离。（网络团队的故障）
- 172 与其他3 个 Akka 节点互相心跳不可达，触发 Unreachable；由于配置了 auto-down-unreachable-after=15s，隔离持续 15s后被自动 down/remove
- 集群发生脑裂，172 与其余节点形成不同视图，Redis 分片状态（cluster:nodes、job:active:*）被污染，任务分配开始失衡。
- 老任务因部分节点本地缓存还在，出现“部分还能跑”；但新增/启停任务只在错误视图(172)生效，由于 172 承载的任务太多压力太大，部分用户看到“到点不执行”。
- 故障处理中多次重启节点，每次都会触发 memberDown/memberUp 重分片；在状态不一致时反复抖动，任务倾斜持续。
- 后续网络恢复后，节点通过 seed 重新入群，继续重启触发事件后，分片逐步收敛，最终恢复。

**发生的问题：**
- 网络分区导致 Akka 脑裂为 2 个集群，api 节点的自动 down 放大了临时故障，导致 Redis 的分片信息与任务分配信息紊乱，且重分片策略与缓存状态叠加，导致任务倾斜。
- dispatch-api 有两类线程都要碰同一批任务缓存数据：
  - 调度线程：按时间扫描并触发任务
  - 刷新线程：从 DB/Redis 同步变更，刷新 jobInfoMap
  - 调度线程和缓存刷新线程共用同一把 ReentrantLock 确保只有一个线程执行，刷新线程在大任务量场景下持锁时间很长（172 当时分到太多任务）。调度线程只能阻塞等待，无法按秒级节奏连续扫描触发。表现出来就是“实例在线、监控也活着，但任务触发间歇甚至停摆”，严重时看起来像不调度。

**根因：**
- 主因：网络隔离 + Akka 自动 down 机制。
- 放大因素：分片状态污染、再平衡不稳、锁竞争导致调度退化。

**解决方案：**
- 增加 trigger 实例心跳到 djs_job_registry（每 1.5s），检测 api 网络不可达的时候要再去判断一下实例心跳，如果最近 10s还有心跳，发送一个告警，暂时不执行 memberDown
- 增加任务倾斜检测与告警（偏差阈值），尽早发现“某节点过载/某节点空载”
- 持续优化重分片与缓存刷新逻辑，减少锁竞争与抖动。首先，现在是先看变更标记（findByChangeJob），有变更才触发重算，不是每轮都全量重分。另外，本地缓存全量刷新改为分页加载（按页拉 jobId 明细），避免一次大批量长时间持锁。

一句话总结：
**这次故障不是单点问题，而是“网络分区触发脑裂”，再叠加自动 down 与分片/缓存机制缺陷；改造重点就是在下线判定前加心跳二次确认，并提升分片与调度稳定性。**
